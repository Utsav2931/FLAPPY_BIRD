...
def main(genomes, config):
    ...
    while (run):
        ...
        for x, bird in enumerate(birds):
            ...
            output = nets[x].activate((bird.y, abs(bird.y - pipes[pipe_ind].height), abs(bird.y - pipes[pipe_ind].bottom), basespeed))
            if output[0] > 0.5 :
                bird.jump()                    
        ...
        for pipe in pipes:
            for x, bird in enumerate(birds):
                ...
                if not pipe.passed and len(birds) and pipe.x < birds[0].x:
                    pipe.passed = True
                    add_pipe = True
        ...
        if add_pipe:
            ...
            for g in ge:
                if score and score % 2 == 0:
                    g.fitness += 0.1
            pipes.append(Pipe(600 + (10 * basespeed) + basespeed))
